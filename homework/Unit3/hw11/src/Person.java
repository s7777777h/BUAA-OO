...
public class Person implements PersonInterface {
    ...
    private final HashMap<Integer, ArrayList<MessageInterface>> messages = new HashMap<>();
    private ListItem messagesHead = null;
    private ListItem messagesTail = null;
    ...
    public List<MessageInterface> getMessages() {
        ...
        ListItem current = messagesHead;
        while (current != null) {
            result.add((MessageInterface) current.getValue());
            current = current.next();
        }
        return result;
    }
    ...
    public void receiveMessage(Message message) {
        if (messagesHead == null) {
            messagesHead = new ListItem(message);
            messagesTail = messagesHead;
        } else {
            messagesHead.insertBefore(new ListItem(message));
            messagesHead = messagesHead.prev();
        }
        if (!messages.containsKey(message.getId())) {
            ArrayList<MessageInterface> newList = new ArrayList<>();
            newList.add(message);
            messages.put(message.getId(), newList);
        } else {
