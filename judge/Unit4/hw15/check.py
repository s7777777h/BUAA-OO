...
class UserState:
    def __init__(self, student_id):
        ...
        self.active_order_fulfilled_date = None
...
class LibraryGroundTruth:
    ...
    def _process_penalties_up_to(self, date_limit):
        ...
        while scan_date < date_limit:
            ...
            for user in self.users.values():
                if user.active_order_isbn and user.active_order_fulfilled_date:
                    for book_copy in self.books_by_isbn.get(user.active_order_isbn, []):
                        if book_copy.location == BookLocation.APPOINTMENT_OFFICE and \
                                book_copy.reserved_for_user_id == user.student_id and \
                                book_copy.reservation_expiry_date == scan_date:
                            self._change_credit(user.student_id, -15)
                            user.active_order_isbn = None
                            user.active_order_fulfilled_date = None
                            break
            scan_date += one_day
    ...
    def _move_book(self, book_copy: BookCopy, new_location_enum: BookLocation, new_holder_id=None,
        ...
        if new_location_enum == BookLocation.APPOINTMENT_OFFICE and reserved_for_user_id:
            ...
            book_copy.reservation_expiry_date = start_date + datetime.timedelta(days=APPOINTMENT_RESERVE_DAYS - 1)
            user = self.get_or_create_user(reserved_for_user_id)
            if user.active_order_isbn == book_copy.isbn: user.active_order_fulfilled_date = start_date
        else:
    ...
    def check_can_pick(self, student_id, isbn_to_pick):
        ...
        for book_copy in self.books_by_isbn.get(isbn_to_pick, []):
            if book_copy.location == BookLocation.APPOINTMENT_OFFICE and str(book_copy.reserved_for_user_id) == str(
                    student_id):
                if book_copy.reservation_expiry_date and self.current_date_obj <= book_copy.reservation_expiry_date: book_to_pick = book_copy; break
        if not book_to_pick: return False, f"No book {isbn_to_pick} at AO validly reserved for {student_id}.", None
    ...
    def handle_student_organization_move(self, date_str, book_full_id, from_loc_str, to_loc_str,
        ...
        if book_copy.location == BookLocation.APPOINTMENT_OFFICE and book_copy.reserved_for_user_id and book_copy.reservation_expiry_date and self.current_date_obj <= book_copy.reservation_expiry_date: return "WA", f"Book {book_full_id} is reserved and cannot be moved."
        if to_loc_str == 'ao':
    ...
    def verify_organization_rules(self, is_open_org_context):
        if is_open_org_context:
            for book_copy in self.books.values():
                ...
                if book_copy.location == BookLocation.APPOINTMENT_OFFICE and book_copy.reservation_expiry_date and self.current_date_obj > book_copy.reservation_expiry_date: return "WA", f"Overdue book {book_copy.full_id} in AO after OPEN org."
...
class StudentProcess:
    ...
    def read_output_line(self):
        ...
        try:
            ...
            if line:
                line = line.strip(); print_recv(line); self.log_buffer.append(f"Student RECV: {line}"); return line
            else:
                self.log_buffer.append(f"Student RECV: [EOF from stdout.readline()]"); return None
        except Exception as e:
                    ...
                    if new_pick_commands:
                        ...
                        if fcb_idx != -1:
                            ...
                            future_command_blocks.append(
                                (target_date_for_pick, new_pick_commands)); future_command_blocks.sort(
                                key=lambda x: x[0])
                        ...
                        if s_status == "accept":
                            ...
                            if gt_ok:
                                ...
                                if book_to_rb and user_to_rb and book_to_rb.location == BookLocation.USER:
                                    ...
                                    user_to_rb.active_order_fulfilled_date = None
                ...
                if borrowed_books_for_potential_return and random.random() < 0.2:
                    ...
                    if next_open_day_date:
                        ...
                        if fcb_idx != -1:
                            ...
                            future_command_blocks.append(
                                (next_open_day_date, deque([return_cmd]))); future_command_blocks.sort(
                                key=lambda x: x[0])
...
def main():
    ...
    if use_generator_choice == 'y':
        try:
            ...
            print_error(f"Could not import '{DATA_GENERATOR_SCRIPT_NAME}.py'."); print_traceback_info()
